import { noop as noop$1, safe_not_equal, subscribe, run_all, is_function, SvelteComponent, init, empty, insert, get_spread_update, get_spread_object, group_outros, transition_out, destroy_component, check_outros, create_component, transition_in, mount_component, detach, assign as assign$1, destroy_each, component_subscribe, create_slot, element, set_attributes, listen, update_slot, compute_rest_props, createEventDispatcher, beforeUpdate, exclude_internal_props, onDestroy } from '../svelte/internal.js';
...
/* node_modules\@inertiajs\inertia-svelte\src\Render.svelte generated by Svelte v3.32.3 */
...
function create_each_block(ctx) {
	let render;
	let current;
	const render_spread_levels = [/*child*/ ctx[3]];
	let render_props = {};

	for (let i = 0; i < render_spread_levels.length; i += 1) {
		render_props = assign$1(render_props, render_spread_levels[i]);
	}
	...
	return {
		...
		p(ctx, dirty) {
			...
			? get_spread_update(render_spread_levels, [get_spread_object(/*child*/ ctx[3])])
			: {};
...
/* node_modules\@inertiajs\inertia-svelte\src\App.svelte generated by Svelte v3.32.3 */
...
function create_fragment$1(ctx) {
	...
	return {
		...
		p(ctx, [dirty]) {
			...
			? get_spread_update(render_spread_levels, [get_spread_object(/*components*/ ctx[0])])
			: {};
...
function instance$1($$self, $$props, $$invalidate) {
	...
	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*$store*/ 64) {
			$$invalidate(5, child = $store.component && h($store.component.default, $store.page.props));
		}

		if ($$self.$$.dirty & /*$store*/ 64) {
			$$invalidate(7, layout = $store.component && $store.component.layout);
		}

		if ($$self.$$.dirty & /*layout, child*/ 160) {
...
/* node_modules\@inertiajs\inertia-svelte\src\Link.svelte generated by Svelte v3.32.3 */
...
function create_fragment$2(ctx) {
	...
	return {
		...
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = listen(a, "click", /*visit*/ ctx[1]);
				mounted = true;
			}
		},
		p(ctx, [dirty]) {
...
function instance$2($$self, $$props, $$invalidate) {
	...
	beforeUpdate(() => {
		...
		const [url, _data] = dist.mergeDataIntoQueryString(method, dist.hrefToUrl(href), data);
	...
	function visit(event) {
		...
		if (dist.shouldIntercept(event)) {
			event.preventDefault();

			dist.Inertia.visit(href, {
				data,
				method,
				preserveScroll,
...
var link = (node, options = {}) => {
  const [url, data] = dist.mergeDataIntoQueryString(options.method || 'get', dist.hrefToUrl(node.href || options.href), options.data || {});
  node.href = url.href;
  options.data = data;
  ...
  function visit(event) {
    ...
    if (dist.shouldIntercept(event)) {
      event.preventDefault();
      dist.Inertia.visit(href, options);
    }
  }
  ...
  return {
    update(newOptions) {
      const [url, data] = dist.mergeDataIntoQueryString(newOptions.method || 'get', dist.hrefToUrl(node.href || newOptions.href), newOptions.data || {});
      node.href = url.href;
...
function remember(initialState, key) {
  const restored = dist.Inertia.restore(key);
  const store = writable(restored !== undefined ? restored : initialState);
  const unsubscribe = store.subscribe(state => dist.Inertia.remember(state, key));
...
export { App, App as InertiaApp, Link as InertiaLink, Link, App as app, link as inertia, Link as link, page, remember };
